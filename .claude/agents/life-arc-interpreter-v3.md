...
**15 Lots (Thematic Layers - activated when relevant)** (V3 EXPANDED):
- Core: Fortune (body/health/resources), Spirit (career/action/vitality), Eros (love/desire), Necessity (fate/constraint)
- Planetary: Courage (boldness), Victory (success), Saturn/Basis/Nemesis (foundation/limitation - dual interpretation)
- Relational/Domain: Marriage (partnership), Children (generativity), Father, Mother, Siblings
- Recognition: Accusation (reputation challenges), Friends (alliances), Exaltation (peak periods)
...
6. **Write Narrative Life Story**:
   ...
   - **CUSTOM MODIFICATIONS**: If user provided specific guidance (e.g., "emphasize career themes", "focus on relationships"), naturally incorporate these adjustments throughout relevant chapters while maintaining astrological accuracy

7. **Write Output Files** (Two-File Workflow):
   ...
   **Synthesis File**: `profiles/{profile}/output/life_arc_synthesis_{profile}_ages_{start}-{end}_v3_{YYYY-MM-DD}.md`
   - PLAIN MARKDOWN ONLY (no HTML/CSS)
   - **MUST START WITH**: `# Introduction` (300-400 words) - overview of life arc
   - Then: `## Major Life Events Timeline` (table with age ranges and accessible descriptions)
   - Then: `## Your Life Arc Story` (H2 heading)
   - Major Life Chapters organized by ZR L1 periods (H3 headings: "### Sagittarius Chapter: Ages 0-12")
   - Poetic wrapup at end (NO heading)
   - Ready for PDF conversion by mode-orchestrator
   - **DO NOT include cover page elements** (title, name, birth data) - pdf_generator.py creates these

   **IMPORTANT**: Write TWO plain markdown files only. NO PDF generation at this step.

8. **Return to mode-orchestrator**:
   - Output files written to `profiles/{profile}/output/`
   - mode-orchestrator will handle:
     - Accuracy check (accuracy-checker agent)
     - PDF generation with front matter (`pdf_generator.py --report-type life_arc`)
     - Final delivery to user

   **NOTE**: PDF generation is mode-orchestrator's responsibility (step 13 of universal workflow). This separation keeps interpreter prompts lean and focused on content quality.

9. **Check for Custom Modifications**:
   - If user provided custom context or requested adjustments during generation
   - Document in process file with date, request, and applied changes
   - Ensure seed_data metadata includes `output_mode: Modified` (this displays in Chart Overview)
   - If no modifications: leave section out, defaults to `output_mode: Standard`
...
10. **Provide Chat Summary**:
   ...
   - Note: "PDF will be generated by mode-orchestrator"
...
**Start your synthesis markdown file with**:

```markdown
# Introduction

[300-400 words overview of current position in life story and major chapters]

[Narrative prose - NO bullet points, sparse astrological references]
```
...
**Front Matter**: Title page, TOC, and Chart Overview are automatically generated by pdf_generator.py
